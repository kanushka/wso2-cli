...
| `wso2 iam apps list \| create <client-id> --type m2m\|public\|federation [--for <url>]` | An OAuth client. An m2m client's generated secret is shown once. A federation client is the confidential client another product signs in through: on the console flow so the CLI's login is its sign-on, callback `<url>/commonauth`, ID token claims `email`, `groups` and `name`; its secret is shown once and the next line names `WSO2_APIM_FEDERATION_CLIENT_SECRET` and the `wso2 apim bootstrap` line, never the value. |
