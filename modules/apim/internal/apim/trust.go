...
// certificate is not trusted by this machine. It carries the host and port the
// certificate came from, which is what the recovery needs and what a plain
// x509 error does not say.
...
func certificateProblem(host string) problem.Problem {
	return problem.New(problem.CategoryProductService, "apim.certificate_untrusted",
		fmt.Sprintf("the certificate API Manager at %s presents is not trusted by this machine", host)).
