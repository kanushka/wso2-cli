...
func TestAnIdentityDeclaringNothingDerivesByScopedRefresh(t *testing.T) {
	...
	if got := document.Contexts[0].Account().Auth.Derivation(); got != contexts.DerivationScopedRefresh {
...
func TestNamingThunderImpliesResourceBoundDerivation(t *testing.T) {
	...
	if got := document.Contexts[0].Account().Auth.Derivation(); got != contexts.DerivationTokenResource {
