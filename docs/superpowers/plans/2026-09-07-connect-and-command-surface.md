...
**Status:** Complete, 2026-09-07. Every task landed; the live matrix
measured six of its seven rows and is recorded in
`docs/research/2026-09-07-one-login-live-matrix.md`. Row 4 (Asgardeo) is
not run — no tenant exists — and access-token expiry was not measured, as
no run spanned an hour. The matrix found one break no unit test could
reach, fixed in `feat(apim): commands ask for no scopes and inherit the
product's recorded scopes`, and the review found five defects, all fixed.
Follow-up work the matrix exposed is listed in section 10 of that
document.
...
- [x] `ProductDescriptor` struct on `Capabilities.Product` (`json:"product,omitempty"`)
  with `Provider`, `IssuerPath`, `ClientID`, `Audience` (`resource`|`client`),
  `DefaultAudience`, `Scopes`, `Grant`, `Machine`.
- [x] `Receipt.Validate` refuses a descriptor whose audience kind is not one of
...
- [x] `fixture.Module.Product *modules.ProductDescriptor` carried into the receipt.
- [x] `modules/iam/module.json`: provider `thunder`, client `wso2-cli`, audience
  ...
  `credential`.
- [x] Tests: a descriptor round-trips through the receipt; each refusal.
- [x] Commit: `feat(modules): carry a product descriptor in the receipt capabilities`.

### Task 2: The login-product pin (`internal/contexts`)

- [x] `Identity.LoginProduct string json:"loginProduct,omitempty"`; validation
  refuses a pin naming a product the identity does not record or one with
  a grant.
- [x] `LoginAccess` uses the pinned product when set, else the sorted order.
- [x] Test: recording an earlier-sorting product moves the login product
  without the pin and does not with it.
- [x] Commit: `feat(contexts): pin the login product on the identity`.

### Task 3: `wso2 <namespace> connect <url>` (`internal/app`)

- [x] `dispatchNamespace`: after `store.Resolve`, when `args[0] == "connect"`,
  run `s.connect(namespace, resolved.Receipt, args[1:])`.
- [x] `connect.go`: a cobra command per invocation (`--identity`,
...
- [x] Tests (fake module store via `fixture.Install`, no network): provider
...
- [x] Commit: `feat(app): add wso2 <namespace> connect, written from the module's product descriptor`.

### Task 4: Bootstraps print the connect line

- [x] `iam bootstrap` prints `wso2 iam connect <issuer>`; `apim bootstrap`
...
- [x] Update the module tests. Commit: `feat(modules): bootstraps print the connect line to run next`.

### Task 5: Empty scope request means the recorded scopes (`internal/auth`)

- [x] In `Broker.Acquire`, before `checkDeclared`: a request with no scopes
  takes `Products[ns].Scopes`. The record stays the ceiling.
- [x] Test in `source_product_test.go`. Drop `--scope` from `apim gateway invoke`.
- [x] Commit: `feat(auth): a module request with no scopes means the product's recorded scopes`.

### Task 6: `--no-input` reaches product commands

- [x] `shellFlags` gains `noInput`; `productLine` carries it; `invokeModule`
  passes it to `sessionEstablisher`, which calls `nonInteractiveControl(noInput)`.
- [x] The module is handed `WSO2_NO_INPUT=1` when either control asked:
  `rpc.Launcher` gains `NoInput bool`, appended to the sanitized environment.
- [x] Tests: productargs reads the flag anywhere; establisher refuses with
...
- [x] Refuse with `auth.organization_switch_unsupported` when the selected
...
- [x] `docs/reference/commands.md`: the `connect` row, `--no-input` on
...
- [x] From an empty home against `cli-thunder3` and `cli-apim`: rows 1, 2,
...
- [x] Run the code review over the branch since `6c4f7d3` and fix what it finds.
